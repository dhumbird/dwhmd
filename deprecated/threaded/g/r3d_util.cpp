...
//**********************************************************************
void TopSurf(config& cfg, float scale, float res, float radsq, float zres){
  float xyres=res;
  int Nx, Ny, i, j; 
  ...
  if (!FileExists(ts_file)){
    ...
    float box[Nx][Ny];
    float maxz=cfg.MaxZ()+sqrt(radsq)+zres;
	...
	while(flag){
	  ...
	  int z=(int)(fabs((R.z - cfg.sLz2)/cfg.Lcz))%cfg.Nz;
	  int a=x+cfg.Nx*y+cfg.Nx*cfg.Ny*z;
	  for (c=cfg.cells[a].nbegin(); c!=cfg.cells[a].nend(); c++)
	    for (j0=cfg.cells[*c].abegin(); j0!=cfg.cells[*c].aend(); j0++)
	      if(flag)
		flag=(R - (*j0)->R).minsqmag(cfg.Lx, cfg.Ly, cfg.Lz) > radsq;
	      else break;
	}
      }
    }
  ...
  for (int i=0; i<Nx-1; i++)
    for (int j=0; j<Ny-1; j++){
      svector v1((i*xyres-cfg.Lx/2)/scale, (cfg.Ly/2-j*xyres)/scale,
		 box[i][j]/scale);
      svector v2((i*xyres-cfg.Lx/2)/scale, (cfg.Ly/2-(j+1)*xyres)/scale,
		 box[i][j+1]/scale);
      svector v3(((i+1)*xyres-cfg.Lx/2)/scale, (cfg.Ly/2-(j+1)*xyres)/scale,
		 box[i+1][j+1]/scale);
      svector v4(((i+1)*xyres-cfg.Lx/2)/scale, (cfg.Ly/2-j*xyres)/scale,
		 box[i+1][j]/scale);
		 
      Plane(v1, v2, v3, v4, pc);
    }
}
