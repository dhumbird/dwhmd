...
void MainRun(int argc, char * argv[]){
  ...
  bool printe=0; bool dump_end=0; bool quiet=0; bool nosput=0;
  string arg;
  for (int i=2; i<argc; i++){
    ...
    else if (arg=="-nosput") nosput=1;
    else CmdError(arg.c_str());
  }
  config cfg(scfg);
  if (To==0.0) cfg.Run(runtime, cfgevery, dt, printe, dump_end,track,\
		       quiet,tso,nosput);
  else cfg.Thermo(To, bhb_dt, cfgevery, dt, printe, dump_end, tso);
  exit(0);
}
