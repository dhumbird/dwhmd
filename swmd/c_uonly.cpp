#include "config.h"
extern double sisi,sisisi, ff, fff, fsi, ffsi, fsisi;

//**********************************************************************
double config::u_SiSi(){
  rija=1/(rij-SW_SI_a);
  if (rija < 0){
    sisi+=SW_SI_SI_EPSA*SpExp(SW_SI_SIGMA*rija)*
...
double config::u_SiSiSi(){
  ...
  if(rika<0 && rjka<0){
    hikj=SpExp(SW_SI_SI_SI_GAMMA*(rika+rjka))
      *pow(cosOikj+ONE_THIRD,2);
  }
  else hikj=0;
  sisisi+=SW_SI_SI_SI_LAMBDA*SW_SI_SI_EPS*(hjik+hijk+hikj);
...
double config::u_FF(){
  ...
  if (rija < 0){
    ff+=SW_F_F_A*SpExp(SW_F_SIGMA*rija)*
...
double config::u_FFF(){
  ...
  if (rija<0 && rika<0){ // jik
    hjik=SW_F_F_F_LAMBDA1*pow(SW_F_F_SIGMA2/rij/rik,4)*
      SpExp(SW_F_GAM1SIG*(rija+rika));
    if (rija2<0 && rika2<0)
      hjik+=(SW_F_F_F_LAMBDA2-cosOjik*cosOjik*SW_F_F_F_LAMBDA3)*
	SpExp(SW_F_GAM2SIG*(rija2+rika2));
  }
  else hjik=0;
  
  if (rija<0 && rjka<0){ // ijk
    hijk=SW_F_F_F_LAMBDA1*pow(SW_F_F_SIGMA2/rij/rjk,4)*
      SpExp(SW_F_GAM1SIG*(rija+rjka));
    if (rija2<0 && rjka2<0)
      hijk+=(SW_F_F_F_LAMBDA2-cosOijk*cosOijk*SW_F_F_F_LAMBDA3)*
	SpExp(SW_F_GAM2SIG*(rija2+rjka2));
  }
  else hijk=0;
  
  if (rika<0 && rjka<0){ // ikj
    hikj=SW_F_F_F_LAMBDA1*pow(SW_F_F_SIGMA2/rik/rjk,4)*
      SpExp(SW_F_GAM1SIG*(rika+rjka));
    if (rika2<0 && rjka2<0)
      hikj+=(SW_F_F_F_LAMBDA2-cosOikj*cosOikj*SW_F_F_F_LAMBDA3)*
	SpExp(SW_F_GAM2SIG*(rika2+rjka2));
  }
  else hikj=0;
  fff+=hjik+hijk+hikj;
...
double config::u_FSi(){
  ...
  if (rija < 0){
    fsi+=SW_SI_F_A*(SW_SI_F_B*pow(SW_SI_SIGMA/rij,3)-pow(SW_SI_SIGMA/rij,2))
