#include "config.h"
extern map<short,double> RC_SQ, LJ_RCSQ;
...
void config::Run(double run_time, int cfgevery, double del_t, bool printe, 
  ...
  while (t<finish){
    ...
    if (track != -1){
      ...
      vector<ljnbr>::iterator sw;
      for (sw=sweet->ljnlist.begin(); sw!=sweet->ljnlist.end(); sw++){
        atom* jj = (atom*)sw->a2;
        cout<<" "<<jj->ix<<" "<<sw->C;
      }
      cout<<endl;
...
//     for (VAI j=begin; j<end; j++){
	  if (i < j && i->ix!=inert && j->ix!=inert){
	    ...
	    R = i->R-j->R; R.minimg(Lx,Ly);
// 	    if ((r=R.sqmag()) < LJ_RCSQ[type]){
// 	      ljnbr ljn;
// 	      ljn.r_true = r; //actually r^2
// 	      ljn.Rhat = R; //actually R
// 	      ljn.type=type;
// 	      ljn.a1=(void*)(&(*i));
// 	      ljn.a2=(void*)(&(*j));
// 	      i->ljnlist.push_back(ljn);
  ...
  for (VAI i=begin; i<end; i++){
	...
	if (i < j && j->ix!=inert){
	  ...
	  if ((r=R.sqmag()) < LJ_RCSQ[type]){
	    ljnbr n;
	    n.r_true = r; //actually r^2
	    n.Rhat = R; //actually R
	    n.type=type;
	    n.a1=(void*)(&(*i));
	    n.a2=(void*)(&(*j));
	    i->ljnlist.push_back(n);
	  }
	}
      }
    }
