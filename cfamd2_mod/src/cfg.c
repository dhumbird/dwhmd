...
static const char * cfg_line_fmt_= 
    "%i\t%s\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%i\t%i\t%i\t%i";
...
void cfg_scan (FILE * fp)
{
    ...
    * first sweep of the input file -- so it's my format. */
	...
	{ 
		...
		#endif
		sscanf(scr_ln, "%i %s %lf %lf %lf %lf %lf %lf %i %i %i %i", 
		    &idum, symStr,
		    &(p->pos->x), &(p->pos->y), &(p->pos->z),
		    &(p->vel->x), &(p->vel->y), &(p->vel->z),
		    &(fixed), &(p->xpass), &(p->ypass), &(p->stat_id));
...
atomPtr cfg_newatom (void)
{
    ...
    pe_set_=0;
    L_[nAtom_].id=nAtom_;
    L_[nAtom_].stat_id=L_[nAtom_-1].stat_id+1;
    return &(L_[nAtom_++]);
}
