...
//***********************************************************************
void InfoCoord(int argc, char * argv[]){
  ...
  while (cfg.t<finish+cfg.dt){
    ...
    for (VPI s=cfg.begin; s<cfg.end; s++){
      if (s->id==cfg.mat && !s->is_fixed){
	for (SPI j=s->nlist.begin(); j!=s->nlist.end(); j++){
	  float rij=(s->R-(*j)->R).minmag(cfg.Lx, cfg.Ly);
	  if ((*j)->id==14 && rij<2.77) s->P.x+=1;
	  else if ((*j)->id==9 && rij<1.81) s->P.x+=1;
	}
      }
    }
    cfg.ForceEval();
