    	...
    	if (refresh)
    	{
      		...
      		for (int run=begin; run<=begin+runs-1; run++)
      		{
	    			...
	    			<<"-----------------------"<<endl;
				{
	  				config cfg(scfg);
	  				int No=cfg.N;
	  				VAI I=cfg.AddIon(e_ion, species, Tion, choose_inc, inc, choose_azi, azi,*seed,0,confine);
	  				int Hix=I->ix;
	  				...
	  				while (!quit)
	  				{
	    				...
	    				else if (I->Nmap[6]+I->Nmap[14] > 0.3)
	    				{
	    					...
	    					cerr<<"* Species appears to be bound to # "<<bondix<<" "<<I->R<<endl;
  	...
  	{ //not fancy
    	if (refresh)
    	{
      		...
      		for (int run=begin; run<=begin+runs-1; run++)
      		{
				...
				{
	  				config cfg(scfg);
	  				cfg.AddIon(e_ion, species, Tion, choose_inc, inc, choose_azi, azi,*seed,0,confine);
